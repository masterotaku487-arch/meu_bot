...
http.createServer((req, res) => {
    res.write("Bot Minecraft AFK Online");
    res.end();
}).listen(8080);
...
function registrar(msg) {
    ...
    // MANTÉM AS COORDENADAS APARECENDO NO CONSOLE
...
function createBot() {
    ...
    // RECONEXÃO AUTOMÁTICA (O CORAÇÃO DO INÍCIO)
    bot.on('error', (err) => {
    ...
    bot.on('end', () => {
        registrar("🔴 Bot desconectado. Reiniciando em 10 segundos...");
        setTimeout(createBot, 10000);
    });
}
...
setInterval(() => {
    if (fs.existsSync(ORDEM_FILE)) {
        try {
            ...
            if (ordem) {
                if (ordem.startsWith('/') || ordem.startsWith('#') || ordem.startsWith('!')) {
                    falar(ordem.startsWith('/') ? ordem : ordem.slice(1));
                }
